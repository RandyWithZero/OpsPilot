...
- `POST /v1/runtime/tasks/{taskID}/callback` accepts running/completed/failed callbacks with an attempt token. Identical terminal callbacks are idempotent; stale tokens and illegal status regressions are rejected.
