...
Environments are project-owned DEV/QA/QE scopes. `GET /v1/environments` supports `project_id`, `type`, `status`, `asset_id`, and `member_id` filters. Environment members must already belong to the project, and environment assets must first be bound to that project through `/v1/projects/{projectID}/assets/{assetID}`. Environment asset/member/file bindings can then be operated through nested `/v1/environments/{environmentID}/...` routes. Deleting an asset or file removes its references from projects/environments; deleting a project cascades its environments.
