...
function filteredWorkflowRuns() {
  ...
  return state.workflowRuns.filter((run) => {
    const text = JSON.stringify({ ...run, workflow: workflowName(run.workflow_id), project: projectName(workflowRunProjectId(run)), trigger: workflowRunTrigger(run), error: workflowRunError(run) }).toLowerCase();
    if (query && !query.split(/\s+/).every((token) => text.includes(token))) return false;
    if (state.filters.status && state.filters.status !== "all" && run.status !== state.filters.status) return false;
    if (state.filters.project_id && state.filters.project_id !== "all" && workflowRunProjectId(run) !== state.filters.project_id) return false;
    return true;
  });
}
...
function workflowRunTable(runs) {
  ...
  return `<table><thead><tr>${headers.map((h) => `<th>${h}</th>`).join("")}</tr></thead><tbody>${runs.map((run) => `<tr>
    ...
    <td>${projectName(workflowRunProjectId(run))}</td>
    <td>${escapeHtml(workflowRunTrigger(run))}</td>
...
function workflowRunDetail(run) {
  return `
    <div class="detail-heading">
      <div><h2>${escapeHtml(run.id)}</h2><p class="muted">${workflowName(run.workflow_id)} · ${projectName(workflowRunProjectId(run))}</p></div>
      ${statusPill(run.status)}
    </div>
    <dl class="kv">
      <dt>触发源</dt><dd>${escapeHtml(workflowRunTrigger(run))}</dd>
    ...
    <h3>错误输出</h3>
    ${workflowRunError(run) ? jsonBlock(workflowRunError(run)) : `<div class="empty-state compact">暂无错误输出。</div>`}
  `;
}

function workflowRunStep(step, index) {
  const type = workflowRunStepType(step);
  const manual = workflowRunStepManual(step);
  return `<li class="${manual ? "manual-step" : ""}">
    <span>${index + 1}</span>
    <div>
      <div class="step-head"><strong>${escapeHtml(step.name)}</strong>${statusPill(step.status)}</div>
      <small>${workflowNodeTypeLabel(type)}${step.assignee_role ? ` · ${displayRole(step.assignee_role)}` : ""}</small>
      <p>${escapeHtml(workflowRunStepOutput(step))}</p>
      ${manual ? `<div class="permission-denied compact"><strong>人工控制节点</strong><span>该步骤必须等待授权角色处理，不能自动跳过。</span></div>` : ""}
    </div>
  </li>`;
}

function workflowRunProjectId(run) {
  return run.project_id || state.workflows.find((workflow) => workflow.id === run.workflow_id)?.project_id || "";
}

function workflowRunTrigger(run) {
  return run.triggered_by || run.trigger_type || "manual";
}

function workflowRunError(run) {
  const stepErrors = (run.steps || []).map((step) => redactSensitiveString(step.error || "")).filter(Boolean);
  return run.error_output || run.error || stepErrors.join("\n");
}

function workflowRunStepType(step) {
  return step.type || step.node_type || step.step_type || "";
}

function workflowRunStepManual(step) {
  return ["approval", "manual"].includes(workflowRunStepType(step)) || step.step_type === "manual" || ["waiting_approval", "blocked"].includes(step.status);
}

function workflowRunStepOutput(step) {
  if (typeof step.output === "string") return step.output || "暂无输出。";
  const output = redactClientPayload(step.output || {});
  return Object.keys(output).length ? JSON.stringify(output) : "暂无输出。";
}
