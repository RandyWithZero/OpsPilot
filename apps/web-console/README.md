...
- API integration through same-origin `/v1/*` foundation endpoints plus explicit local mock / development-header fallback for offline checks
...
- The login form requires a non-empty password and submits it as `password` / `credential` for the IAM contract. Empty credentials are blocked before the console shell can load.
